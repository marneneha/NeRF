...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        ray_bundle_ray_sampler = get_rays_from_pixels(xy_grid, image_size, camera) # TODO (1.3): implement in ray_utils.py

        # TODO (1.3): Visualize xy grid using vis_grid
        if cam_idx == 0 and file_prefix == '':
           grid_img =  vis_grid(xy_grid=xy_grid,image_size=image_size)
           plt.imsave('images/1.3-grid.png',grid_img)
        # TODO (1.3): Visualize rays using vis_rays
        if cam_idx == 0 and file_prefix == '':
            ray_img = vis_rays(ray_bundle=ray_bundle_ray_sampler,image_size=image_size)
            plt.imsave('images/1.3-ray.png', ray_img)        

        # TODO (1.4): Implement point sampling along rays in sampler.py
        ray_bundle_ray_sampler = model.sampler(ray_bundle_ray_sampler)
 
        # TODO (1.4): Visualize sample points as point cloud
        if cam_idx == 0 and file_prefix == '':
            render_points('images/1.4_samples_pc.png', ray_bundle_ray_sampler.sample_points.reshape(1, -1, 3))

        # TODO (1.5): Implement rendering in renderer.py
        out = model(ray_bundle_ray_sampler)
...
def train(
    ...
    for epoch in t_range:
        for iteration, batch in enumerate(train_dataloader):
            ...
            # TODO (2.1): implement in ray_utils.py
